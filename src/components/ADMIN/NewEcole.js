...
const EcoleRegistrationForm = ({ isOpen, toggle }) => {
  ...
  const addEcole = async (userId) => {
    ...
    try {
      await axios.post(ecolesApi, { ...formData, userId });
      toggle();
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const data = (await axios.post(registerApi , {email: formData.emailEcole , password: formData.password , role: 'ecole'})).data
      const userId = data._id
      await addEcole(userId);
    } catch (error) {
